...
if [ "$MODE" == "frontdoor" ]; then
    ...
    curl -sS "http://$LIVINGROOMCAM_IP/cgi-bin/CGIProxy.fcgi?cmd=snapPicture2&usr=$LIVINGROOMCAM_USER&pwd=$LIVINGROOMCAM_PASSWORD&" -o tmp.jpg 2>$LOGDIR/camera.$MODE.log
